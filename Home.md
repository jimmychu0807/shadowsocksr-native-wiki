...
* 向 `朝鲜` 🇰🇵 看齐, 偌大中国只有几十数百个国外 `IP` 可以访问, 其它一概封杀. 那, 我也没办法了, 还翻什么墙啊? 我们——被剥夺了基本 `通讯自由` 以至于上个破网都是犯罪的十几亿猪民——是不是有比宅家里猥琐地翻墙更重要的事要做? (拥有这数百 `IP` 管理权的 `上帝` 们, 别犹豫, 赶紧把 `SSRoT` 部署起来, 这是你们一夜暴富的最后机会.)
