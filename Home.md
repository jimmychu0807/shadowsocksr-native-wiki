...
2. 交换的数据可以经过 `SS`/`SSR` 加密, 甚至不用加密, 因为 `HTTPS` 协议传输内容, 包括请求路径`URL`, 数据等, 都是经过强加密的, `GFW` 破解不了, 即使 `GFW` 牛逼到真能破解了, 那 `HTTPS` 协议本身也该升级换代了.
...
5. 与 `反向代理` web 服务器进行通讯的 `SSRoT` 服务器也是基于 `WebSocket` 协议, 然后将数据转发到用户试图访问的 `目标网站`.
