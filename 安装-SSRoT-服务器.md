...
- 大家可以看到, 该节区有句 `proxy_pass http://127.0.0.1:10000;` 表明, 为 `反代` 提供服务的 `SSR` 服务器必须监听在 `127.0.0.1:10000` 上, 否则不能联动. 这也表明 `SSR` 服务器也可以安装在另一台主机上的另一端口, 只要把这里的 `proxy_pass` 值设置正确就没问题.
